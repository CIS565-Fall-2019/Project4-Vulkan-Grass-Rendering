...
void Renderer::CreateComputeDescriptorSetLayout() {
	...
	// - modeling this off of the above function plus Vulkan-1.1-web.pdf and talking 
	//	with others on how vulkan works

	// Describe the binding of the descriptor set layout of each
	...
	VkDescriptorSetLayoutBinding cullBinding = {};
	cullBinding.binding = 1;
	cullBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
	cullBinding.descriptorCount = 1;
	cullBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
	cullBinding.pImmutableSamplers = nullptr;
	...
	std::vector<VkDescriptorSetLayoutBinding> bindings = { inputBinding,
														   cullBinding,
														   numBinding };
...
void Renderer::CreateGrassDescriptorSets() {
	...
	// Based on the ModelDescriptorSets function above
	grassDescriptorSets.resize(scene->GetBlades().size());

	// Describe the desciptor set
	VkDescriptorSetLayout layouts[] = { modelDescriptorSetLayout }; // Layout works for both
	VkDescriptorSetAllocateInfo allocInfo = {};
	allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
	allocInfo.descriptorPool = descriptorPool;
	...
	for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
		...
		grassBufferInfo.range = sizeof(ModelBufferObject); // Thanks Josh's piazza post!

		// No need to bind image resources, so the following code (and size) is halved
...
void Renderer::CreateComputeDescriptorSets() {
	...
	// Like grass, based on the ModelDescriptorSets function above
	...
	for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
		// Bind input blades to the descriptor
		VkDescriptorBufferInfo inputBufferInfo = {};
		inputBufferInfo.buffer = scene->GetBlades()[i]->GetBladesBuffer();
		inputBufferInfo.offset = 0;
		inputBufferInfo.range = /*scene->GetBlades().size()*/ NUM_BLADES * sizeof(Blade);
		...
		descriptorWrites[3 * i].pBufferInfo = &inputBufferInfo;
		descriptorWrites[3 * i].pImageInfo = nullptr;
		descriptorWrites[3 * i].pTexelBufferView = nullptr;

		// Bind culled blades to the descriptor
		...
		cullBufferInfo.range = /*scene->GetBlades().size()*/ NUM_BLADES * sizeof(Blade);
		...
		// Bind number of remaining blades to the descriptor
...
void Renderer::RecordComputeCommandBuffer() {
	...
	for (int i = 0; i < computeDescriptorSets.size(); i++) {
		...
		vkCmdDispatch(computeCommandBuffer, (int) ceil((float) NUM_BLADES / (float) WORKGROUP_SIZE), 1, 1);
	}

	//std::cout<< NUM_BLADES << std::endl;
	//std::cout << scene->GetBlades().size() << std::endl;
...
void Renderer::RecordCommandBuffers() {
    ...
    for (size_t i = 0; i < commandBuffers.size(); i++) {
        ...
        for (uint32_t j = 0; j < scene->GetBlades().size(); ++j) {
			...
			vkCmdBindDescriptorSets(commandBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS, /*grassPipelineLayout*/graphicsPipelineLayout, 1, 1, &grassDescriptorSets[j], 0, nullptr);
