...
void Renderer::CreateRenderPass() {
	...
	renderPassInfo.attachmentCount = static_cast<uint32_t>(attachments.size());//2
	renderPassInfo.pAttachments = attachments.data();//color and depth
...
void Renderer::CreateModelDescriptorSetLayout() {
	...
	uboLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
	uboLayoutBinding.descriptorCount = 1;
	uboLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;//use only in vertex
	uboLayoutBinding.pImmutableSamplers = nullptr;
...
void Renderer::CreateComputeDescriptorSetLayout() {
	...
	std::vector<VkDescriptorSetLayoutBinding> bindings = { BladesLayoutBinding, culledLayoutBinding, NumberLayoutBinding };
...
// a descriptor set for each VkBuffer resource to bind it to the uniform buffer descriptor.
void Renderer::CreateGrassDescriptorSets() {
	...
	// Describe the desciptor set
	VkDescriptorSetLayout layouts[] = { modelDescriptorSetLayout };//model matrix of grass
	VkDescriptorSetAllocateInfo allocInfo = {};
	allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
	allocInfo.descriptorPool = descriptorPool;//pool pointer
	allocInfo.descriptorSetCount = static_cast<uint32_t>(grassDescriptorSets.size());//blades size
	allocInfo.pSetLayouts = layouts;
...
void Renderer::CreateComputeDescriptorSets() {
	...
	allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
	allocInfo.descriptorPool = descriptorPool;//pool pointer
	allocInfo.descriptorSetCount = static_cast<uint32_t>(computeDescriptorSets.size());//compute descriptor size
	allocInfo.pSetLayouts = layouts;
	...
	//3: Storage buffers, culled grass blades, and the output number of grass blades 
	...
	for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
		...
		outputBufferInfo.buffer = scene->GetBlades()[i]->GetNumBladesBuffer();
		outputBufferInfo.offset = 0;
		outputBufferInfo.range = sizeof(BladeDrawIndirect);
		...
		descriptorWrites[3 * i + 2].pBufferInfo = &outputBufferInfo;
	...
	for (size_t i = 0; i < commandBuffers.size(); i++) {
		...
		for (uint32_t j = 0; j < scene->GetModels().size(); ++j) {
			...
			vkCmdBindIndexBuffer(commandBuffers[i],
				scene->GetModels()[j]->getIndexBuffer(), 0, VK_INDEX_TYPE_UINT32);

			// Bind the descriptor set for each model
			vkCmdBindDescriptorSets(commandBuffers[i],
				VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipelineLayout, 1, 1,
		...
		for (uint32_t j = 0; j < scene->GetBlades().size(); ++j) {
			...
			vkCmdDrawIndirect(commandBuffers[i],
				scene->GetBlades()[j]->GetNumBladesBuffer(), //buffer
				0, //offset
				1, //drawcount
				sizeof(BladeDrawIndirect));//stride
		}
